...
class ExitingElevatorState(IRobotLiftState):
    def on_enter(self):
        ...
        self.context.get_logger().info(f"Driving {EXIT_DISTANCE} meter forward out of elevator")
    ...
    def execute(self):
        ...
        if not self._nav_succeeded:
            raise RuntimeError(f"DriveOnHeading failed: could not drive {EXIT_DISTANCE} meter forward")
        return None
